...
public class GraphView extends View {
    ...
    private void setTypes(Context context, AttributeSet attrs) {
        ...
        mPointPaint.setColor(types.getColor(R.styleable.GraphView_pointColor, 0xFF147C02));
        mPointSize = (int) types.getDimension(R.styleable.GraphView_pointSize, 30);
        mPointRadius = mPointSize / 2;

        mLineColor = types.getColor(R.styleable.GraphView_lineColor, Color.GREEN);
        mThickness = types.getDimension(R.styleable.GraphView_lineThickness, 3);
    }

    //그래프 정보를 받는다
    public void setPoints(ArrayList<Integer> points, int unit, int origin, int divide) {
        int[] points_array = new int[points.size()];
        for(int i=0;i<points.size();i++)
            points_array[i] = points.get(i);
        mPoints = points_array;   //y축 값 배열
        mUnit = unit;       //y축 단위
